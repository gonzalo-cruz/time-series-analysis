...
\title{\textbf{Práctica Final:\\Análisis y Predicción de Series Temporales}\\[0.5em]
\large Competición Kaggle -- Universidad Rey Juan Carlos (URJC)}
\author{Gonzalo C. Gómez}
\date{Mayo 2026}
...
\subsubsection*{Motivación del enfoque}
...
\]
donde $t$ es el día del año (1--365). Con $K=5$ se tienen 10 regresores que representan los primeros cinco armónicos de la variación anual. La ventaja de este enfoque es que estos regressores se calculan únicamente a partir del índice de fecha, sin usar los valores de la serie, así que no hay fuga de información al construirlos para 2003. El modelo SARIMAX con $m=7$ y Fourier $K=5$ separa entonces las dos fuentes de variación: el componente estocástico semanal (manejado por el SARIMA) y el patrón anual determinista (manejado por el Fourier).
...
La serie de precios es heterocedástica: cuando el precio es alto, también lo son las oscilaciones diarias. Aplicando el test de Levene \cite{levene1960} dividiendo la serie en 4 bloques, el p-valor es prácticamente cero.
...
Lo que quedaría por explorar es modelar la volatilidad con un GARCH$(1,1)$ \cite{engle1982,bollerslev1986}, que mejoraría los intervalos de predicción aunque no cambie la predicción puntual.

\subsection{Serie B -- Exploración con Gradient Boosting}

Como exploración adicional, probé un modelo \textbf{HistGradientBoosting} (scikit-learn) para la serie B. Las features usadas fueron: día de la semana, mes, día del año, índice de año, términos de Fourier semanales y anuales, y tres lags anuales (lag$_{364}$, lag$_{365}$, lag$_{366}$, el mismo día del año anterior $\pm 1$ día).

La ventaja de este enfoque es que el lag del año anterior aporta el nivel base de cada día directamente, y el árbol de gradiente puede capturar interacciones no lineales entre el día de la semana y el mes sin necesitar diferenciación. En validación sobre el año 2002, el RMSE fue de 514, frente al RMSE de 710 del SARIMA+Fourier.

No se usó para la predicción final porque el objetivo de esta práctica era explorar los modelos SARIMA/SARIMAX. De todas formas el resultado apunta a que para series con dependencias de nivel interanuales, los modelos basados en árboles con lags anuales pueden ser una alternativa interesante.

\subsection{Serie D -- Transformación Box-Cox}

La transformación Box-Cox \cite{boxcox1964} elige automáticamente el exponente que mejor estabiliza la varianza. La serie D crece bastante así que pensé que podría haber heterocedasticidad, pero el test de Levene dio $p=0.31$: la varianza ya era constante. El lambda óptimo resultó $\lambda=0.35$, entre logaritmo y raíz cuadrada.
...
\textit{An analysis of transformations}.
Journal of the Royal Statistical Society B, \textbf{26}(2), 211--252.

\bibitem{engle1982}
Engle, R. F.\ (1982).
\textit{Autoregressive conditional heteroscedasticity with estimates of the variance of United Kingdom inflation}.
Econometrica, \textbf{50}(4), 987--1007.

\bibitem{bollerslev1986}
Bollerslev, T.\ (1986).
\textit{Generalized autoregressive conditional heteroscedasticity}.
Journal of Econometrics, \textbf{31}(3), 307--327.
...
\bibitem{levene1960}
Levene, H.\ (1960).
\textit{Robust tests for equality of variances}.
In Olkin, I.\ et al.\ (Eds.), \textit{Contributions to Probability and Statistics}, pp.\ 278--292.
