...
El objetivo de esta práctica es predecir cuatro series temporales diferentes para la asignatura de análisis de series temporales. Para cada una se ha seguido el mismo proceso: análisis exploratorio, test de estacionariedad, selección del modelo por RMSE en validación y predicción final reajustando sobre todos los datos disponibles.
...
El test ADF da $p=0.74$, así que la serie no es estacionaria. Con la primera diferencia cae a $p<0.0001$, lo que confirma integración de orden 1. El ACF y PACF de la serie diferenciada no muestran autocorrelaciones relevantes, lo que es coherente con un precio que sigue aproximadamente un paseo aleatorio.
...
La serie B contiene 3287 observaciones diarias de nacimientos (1994-2002). Tiene dos estacionalidades claramente visibles: una semanal muy marcada (pocos nacimientos en fin de semana) y una anual (pico en agosto-septiembre, mínimo en enero, diferencia de unos 1000 nacimientos/día).
...
\]
donde $t$ es el día del año (1--365). Con $K=5$ se tienen 10 regresores que representan la variación anual. La ventaja de esto es que estos regresores se calculan únicamente a partir del índice de fecha, sin usar los valores de la serie, así que no hay fuga de información al construirlos para validación. El modelo SARIMAX con $m=7$ y Fourier $K=5$ separa entonces las dos fuentes de variación: el componente aleatorio semanal (manejado por el SARIMA) y el patrón anual determinista (manejado por el Fourier).
...
El mejor modelo fue \textbf{SARIMA$(1,0,0)(0,1,1)_{12}$} con un \textbf{regresor de tendencia lineal} (RMSE$=0.287$ en validación, últimos 24 meses). La tendencia lineal exógena modela el calentamiento global continuo de forma explícita y la diferenciación estacional $D=1$ elimina el patrón mensual.
...
La serie D tiene 132 observaciones mensuales (1980-1990). Los valores crecen de forma sostenida (de $\approx 464$ a $\approx 2286$) con una estacionalidad anual clara.
...
En finanzas muchas veces se  trabaja con \textbf{log-retornos} ($r_t = \log P_t - \log P_{t-1}$), que son estacionarios y mucho más homocedásticos. Ajusté un ARIMA sobre los retornos y luego deshice la transformación para obtener precios. El modelo resultante fue ARIMA$(0,0,0)$ (ruido blanco), lo cual tiene sentido teniendo en cuenta lo impredecible que es el mercado.
...
Para la serie B, una línea interesante sería usar \textbf{Gradient Boosting} como modelo principal. Los resultados en validación nos sugieren que el modelo puede no estar capturando las no linealidades de manera correcta. Utilizando dicho modelo y haciendo una búsqueda de hiperparametros correcta podríamos mejorar dicha predicción. Asimismo, podríamos utilizar datos generados para dotar a los modelos de un mayor número de observaciones, lo que podría mejorar su capacidad de generalización.
